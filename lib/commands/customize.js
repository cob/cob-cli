...
async function applyCustomization(customizationRepo) {
  ...
  if (customization.actions) {
    customization.actions(customizationRepo.name, answers, copy);
  } else {
    // Default actions
    copy(customizationRepo.name, answers);
  }
...
/* ************************************************************************ */
async function copy(customizationRepoName, substitutions = {}) {
  ...
  ncp(
    ...
    (error) => {
      if(error) {
        ...
        return mergeFiles(customizationRepoName);
      }
    }
  )
}
