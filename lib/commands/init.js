...
const { getServer } = require("../task_lists/common_enviromentManager");
...
async function init(args) {
    ...
    const projectName = "server_"+env.serverName;
    ...
    } else if( projectPathOkFor(projectName) && await notInsideGitRepo() && await validCobServer(env.server)) {
        console.log("\nCreating project", projectName.bold.blue );
        initTasks = newProjectTasks(env, projectName, repo, args )
    } else {
        return //Error
    }
