...
async function updateFromServer(args) {
    try {
        ...
        if (args.code) {
            ...
            const customizations = await readConfig("customizationsDef.yml")
            ...
            await fs.writeFile('others/customizationData/_index', customizations.map(c => c.name).join('\n'), 'utf-8')
        }
...
async function readConfig(confFileName) {
    try {
        const data = await fs.readFile(confFileName, 'utf-8');
