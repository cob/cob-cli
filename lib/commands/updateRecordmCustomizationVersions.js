...
const { validateUpdateCustomizationVersionsConditions } = require("../task_lists/updateRecordmCustomizationVersions_validate");
...
async function updateRecordmCustomizationVersions(args) {
    ...
    try {
        ...
        await validateUpdateCustomizationVersionsConditions(cmdEnv).run()

        const fs = require('fs');
        axios.defaults.headers.common['Cookie'] = await getRecordmToken(fs);

        //Read local customization version
        var localCustomizations;
        try {
            localCustomizations = JSON.parse(fs.readFileSync(`${process.cwd()}/customizations.json`, 'utf8'));
        } catch (error) {
            console.error("\n",error.message)
        }
        ...
        const prompt = await inquirer.prompt([
            {
                name: 'githubLatest',
                message: 'Quer atualizar as últimas versões do GITHUB para o Recordm ? (yes/no)'
            }
        ])

        if (prompt.githubLatest == "yes"){
...
//Auxliary functions
async function postLocalVersionsNotOnRecordm(localVersions,remoteVersions,repoId){
    let versionsUpdated = []
    ...
    if(versionsNotFoundOnRecordm.length > 0){
        ...
        for (const index in localVersionsNotRegistered) {
            ...
            const result = await axios.post(CONCURRENT_SCRIPT_URL,requestDataRepoCustomization(repoId,record.instanceId));
            if (result.data.id > 0 ) {
    ...
    console.log("Local Versions UPDATED to RecordM Definition <Repositório-Customizações>: ",versionsUpdated)
    console.log("Local Versions not Updated: ",difference)
}
...
function requestDataRepoCustomization(repoId,customizationVersionInstanceId){
    return {
        "saveToRecordm":true,
        "repositório":repoId,
...
async function getRepoId(cmdEnv) {
    ...
    if ( maquinaIdResponse.length > 0 ){
        repoId = maquinaIdResponse[0]._source.instanceId;
    } else {
        repoId = ( await axios.post(CONCURRENT_SCRIPT_URL,requestDataRepositorios(cmdEnv.server) ) ).data.id
    }
    return repoId
}
