...
async function upgradeRepo() {
    ...
    try {
        await new Listr([
            {   title: "Get current Version of repo".bold,                                                    task: ctx => ctx.version = _getCurrentVersion() },
            ...
            {   title: "Upgrade 0 to 1",                       enabled: ctx => ctx.version < 1,               task: ()  => _upgrade0to1() },
            {   title: "Upgrade 1 to 2",                       enabled: ctx => ctx.version < 2,               task: ()  => _upgrade1to2() }
        ]).run()
...
function checkVersion() {
    let repoVersion = _getCurrentVersion();
    if(repoVersion == 9999 ) {
...
function _upgrade1to2() {
    fs.writeFileSync(VersionFilename, "2.0.0")
}