...
const REPO_VERSION = 255 // por convenção deverá ser actualizado com versionStr2Number( require('../../package.json').version ) sempre que necessário, tendo depois a função necessária para fazer o upgrade
...
async function upgradeRepo() {
    ...
    try {
        await new Listr([
            ...
            {   title: "Upgrade 2.1 to 2.55".bold,             enabled: ctx => ctx.version < 255,             task: ()  => setVersion() }
        ]).run()
...
/* ************************************ */
function setVersion() {
    fs.writeFileSync(VersionFilename, String(REPO_VERSION))
}
