...
const versionStr2Number = (versionStr) => versionStr.split(".").reduce( (x,y) => Number(x)*100+Number(y))
const CURRENT_VERSION = versionStr2Number( require('../../package.json').version )

async function upgradeRepo() {
    console.log("Checking conditions to upgrade cob-cli repo (current version " + CURRENT_VERSION + ")" );
    try {
        await new Listr([
            {   title: "Get repo Version",                                                                    task: ctx => {ctx.version = _getRepoVersion();console.log(">>>"+ctx.version) }},
            ...
            {   title: "Upgrade 2.0.x to 2.1.0".bold,          enabled: ctx => ctx.version < 20100,           task: ()  => _upgrade2to2_1() }
        ]).run()
