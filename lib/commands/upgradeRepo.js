...
async function upgradeRepo() {
    ...
    try {
        await new Listr([
            ...
            {   title: "Check HEAD is not dettached",          enabled: ctx => ctx.version < CURRENT_VERSION, task: ()  => getCurrentBranch().then( currentBranch => { if(!currentBranch) throw new Error("Aborded:".red + " git head is dettached")} ) }, 
            {   title: "Update with origin: git fetch origin", enabled: ctx => ctx.version < CURRENT_VERSION, task: ()  => git().fetch()   },
            {   title: "Check git status",                     enabled: ctx => ctx.version < CURRENT_VERSION, task: ()  => checkWorkingCopyCleanliness().catch( err => {throw err} ) }, 
