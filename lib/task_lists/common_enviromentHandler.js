...
const { getSshArgs, sshConnectionError } = require("./common_helpers")
...
async function getCurrentCommandEnviroment(args, newServername) {
    ...
    // On init (newServername) there's nothing deployed yet, and the init tasks check connectivity themselves
    const lastEnvironmentDeployedToServer = (args.localOnly || newServername) ? environmentName : await _getLastEnvironmentDeployed(server)
...
/* ************************************ */
function _validateServername(servername) {
    // FQDNs and IPs are used as-is (see _getServerSSHFQDN), only short names get validated
    if (servername.includes(".") || !servername.includes("_")) return;

    let msg = "\nError: ".red + "invalid servername " + servername.blue.bold + ": hostnames can't contain '_'.\n"
    msg += "Use only the server name, without the " + "server_".bold + " prefix and without " + ".cultofbits.pt".bold
    const suggestion = servername.replace(/^server_/, "")
    if (suggestion != servername && !suggestion.includes("_")) msg += ", i.e. " + suggestion.blue.bold
    throw new Error(msg + ".\n")
}
...
async function _getLastEnvironmentDeployed(server) {
    try {
        ...
        if (error.exitCode === 255) {
            throw sshConnectionError(server, error);
        }
        return "prod"
    }
}
