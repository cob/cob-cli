...
function checkConnectivity(server) {
    ...
    return execa('ssh', ["-o", "StrictHostKeyChecking=no", "-o", "PreferredAuthentications=publickey", "-o", "BatchMode=yes", "-o", "ConnectTimeout=15", server, "test -w /etc/recordm/recordm.rc"])
}
...
async function sudo(server, commands=[], observer, isVerbose){
        ...
        try {
            ...
            if(e.stderr.includes("no password was provided")
               ...
               || e.stderr.includes("a password is required")){
                ...
                if(isVerbose) console.error("Error executing commands with 'sudo'", commands, e);
