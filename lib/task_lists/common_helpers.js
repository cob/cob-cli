...
/* ************************************ */
// ssh exits with 255 whenever the connection itself fails (unknown host, no key, refused...).
// execa's shortMessage only has the exit code and the command, the actual reason is on stderr.
function sshConnectionError(server, error) {
    const reason = (error.stderr || "").trim()
    return new Error(
        `SSH connection to '${server}' failed (exit code ${error.exitCode})` +
        (reason ? ":\n\t" + reason.split("\n").join("\n\t") : "") +
