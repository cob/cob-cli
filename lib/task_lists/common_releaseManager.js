...
async function registerRelease(server) {
    ...
    if (diffs) {
        ...
        await execa('ssh', [server, "echo '# " + newDeployText + diffs + "\n\n' >> " + SERVER_CHANGELOG])

        var changelogFileName = (currentBranch != "master" ? currentBranch+"-branch." : "") + CHANGELOGFILE
        var originalChangeLog = "";
        try {
            originalChangeLog = fs.readFileSync(changelogFileName).toString(); //TODO: update file for server also
            debugger;
        ...
        await fs.writeFile(changelogFileName, newChangeLog, function (err) {
            if (err) {
