...
const { getServerName } = require("./common_helpers");
            ...
            if (gitLog.all.length) {
                for (let i = 0; i < gitLog.all.length; i++) {
                    if (gitLog.all[i].message.startsWith("Deploy ")) {
                        ...
                        // Match string above needs to be in accordance with releaseSignature() beggining string
                        continue;
                    }
                    diffs = diffs
                        + "\n * "
...
function releaseSignature(branch, sha, server) {
    let signature = 
        ...
        + (server != getServerName() ? " to " + server : "")
