...
/* ************************************ */
function copyFiles(env, from, to) {
    return _syncFiles(_syncFiles.COPY, env, from, to);
} 
exports.copyFiles = copyFiles;

/* ************************************ */
async function testEquality(env, from, to) {
    return (await _syncFiles(_syncFiles.TEST, env, from, to)).filter( f => !f.endsWith("/"))
}
exports.testEquality = testEquality;

/* ************************************ */
async function _syncFiles(executionType, env, from, to, extraOptions = []) {
    let changes = [];
    let requests = COB_PRODUCTS.map(product => {
        return new Promise(async (resolve, reject) => {
            let fromPath = resolveCobPath(env.server, from, product);
            let toPath = resolveCobPath(env.server, to, product);
...
/* ************************************ */
async function confirmExecutionOfChanges(env, extraOptions = []) {
    console.log("\nChecking changes... ")
    let changes = (await _syncFiles(_syncFiles.DIFF, env,  "localCopy", "serverLive", extraOptions)).filter( f => !f.endsWith("/") )
    if (changes.length) {
        console.log("\nChanges that will be done in " + env.serverStr + ":");
