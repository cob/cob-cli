            ...
            while (count < RSYNC_RETRIES) {
                ...
                await execa('rsync', rsyncArgs, {
                    ...
                    .then((value) => {
                        ...
                        if (executionType == _syncFiles.TEST)
...
function _filterDecryptedEncFiles(product, rsyncOutput) {
    return rsyncOutput
        .split("\n")
        .filter(line => {
            if (!line.startsWith("*deleting ")) return true;
...
async function decryptEncFiles(cmdEnv, encFiles) {
    ...
    const cmd = encFiles.map(f => `sudo -u cob-secrets /usr/local/bin/cob-decrypt '${f}'`).join(" && ");
    await execa('ssh', [...getSshArgs(cmdEnv.server), cmd]);
}
