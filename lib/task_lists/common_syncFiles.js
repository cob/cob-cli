...
/* ************************************ */
// Server live paths of every local .enc file across all products. Unlike the
// encFiles returned by copyFiles (only the ones rsync changed), this lists them
// all — used to force-decrypt every .enc file on `deploy -f`.
function listEncFileServerPaths(cmdEnv) {
    const products = cmdEnv.products.length ? cmdEnv.products : COB_PRODUCTS;
    return products.flatMap(product => _getAllEncFileServerPaths(product));
}
exports.listEncFileServerPaths = listEncFileServerPaths;

/* ************************************ */
function _getAllEncFileServerPaths(product) {
    const productDir = "./" + product;
    if (!fs.existsSync(productDir)) return [];
    const basePath = _getServerLiveBasePath(product);
    const paths = [];
    // exclude ENV files, they are never copied to servers (rsync excludes them)
    const ENV_SPECIFIC_RE = /\.ENV__.*__\./;
    const walk = (dir, rel) => {
        for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
            const entryRel = rel ? rel + "/" + entry.name : entry.name;
            if (entry.isDirectory()) walk(path.join(dir, entry.name), entryRel);
            else if (entry.name.endsWith(".enc") && !ENV_SPECIFIC_RE.test(entry.name)) paths.push(basePath + entryRel);
        }
    };
    walk(productDir, "");
    return paths;
...
async function deploySolutions(server) {
    // stdio:'inherit' gives cc-install the real terminal so it can prompt
