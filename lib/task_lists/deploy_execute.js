...
function executeTasks(server, currentBranch, args) {
    ...
    return new Listr([
        {title: "Register release in CHANGELOG.md", task: () => registerRelease(server, currentBranch)},
        {title: `git push (branch: ${currentBranch})`, task: () => git().push("origin", currentBranch, {"--set-upstream": null})},
