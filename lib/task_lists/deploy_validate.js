...
function validateTasks(serverName, args) {
    ...
    return new Listr([
        ...
        {   title: "Check latest tag == server copy",                    skip: () => args.force,  task: () => syncFiles(server, "localMaster", "productionMaster", syncFiles.TEST) },
            ...
            task: (ctx,task) => 
                ...
                .catch((err) => { 
                    if(err.stderr) {
                        ...
                        git().checkout("master"); // get back to original branch
                        let errors = [
