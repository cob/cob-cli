...
const { IN_PROGRESS_TEST_FILE } = require("./test_otherFilesContiousReload")
...
function validateTasksForDeploy(server, args) {
    ...
    return new Listr([
        ...
        {   title: "Check git status",                                                 task: () => checkBranchCleanliness("master") }, 
        {   title: "git checkout latest tag",                  skip: () => args.force, task: () => git().checkoutLatestTag() }, 
        {   title: "Check latest tag == server copy",          skip: () => args.force, task: () => testEquality(server, "localMaster", "productionMaster") },
        ...
        {   title: "Check there're no 'cob-cli test' running", skip: () => args.force, task: () => checkNoTestsRunning(server) },
    ])
}
...
async function checkBranchCleanliness(branch) {
    await git().status()
    .then(result => {
...
/* **************************************************************************************** */
function checkNoTestsRunning(server) {
    return execa('ssh', [server, "cat " + IN_PROGRESS_TEST_FILE])
            .then(value => { 
                throw new Error(
