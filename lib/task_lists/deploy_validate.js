...
function validateDeployConditions(env, args) {
    return new Listr([
        ...
        {   title: "Check there're no 'cob-cli test' running on server".bold,                                           task: ()     => checkNoTestsRunningOnServer(env.server) },
        ...
        {   title: "find out SHA for last-deploy on specified server",         skip: ctx => args.force,                 task: (ctx)  => getLastDeployedSha(env.server).then(lastSha => _handleGetLastDeployedSha(ctx, env, lastSha)) }, 
        {   title: "git checkout SHA-for-last-deploy",                         skip: ctx => args.force || !ctx.lastSha, task: (ctx)  => git().checkout(ctx.lastSha) }, 
        {   title: "Check last-deploy == serverLive".bold,                     skip: ctx => args.force || !ctx.lastSha, task: (ctx)  => testEquality(env, "localCopy", "serverLive").then( changes => _handleTestEquality(ctx, changes)) },
...
/* ************************************ */
function _handleGetLastDeployedSha(ctx, env, lastSha) {
    if (lastSha) { 
        ctx.lastSha = lastSha 
    } else { 
        ctx.err = 
            ...
            + ("cob-cli deploy -s " + env.serverName + " --force").yellow 
