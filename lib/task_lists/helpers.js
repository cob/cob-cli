...
async function _syncFiles(executionType, server, from, to, extraOptions = []) {
    ...
    let requests = COB_PRODUCTS.map( product => {
        return new Promise( async (resolve, reject) => {
            ...
            if(from != "productionLive" && executionType == _syncFiles.COPY && !fs.existsSync(product)) {
                resolve()
                return;
            }
...
function setupGitHooks() {
    ...
    const gitHookFile = ".git/hooks/commit-msg";

    /* both @commitlint cli and configuration are dependencies of the cob-cli package. Using the full path to them. */
    fs.writeFileSync(gitHookFile, (windows ? "#!C:/Program\ Files/Git/usr/bin/sh.exe" : "#!/bin/sh")
                                   + "\n\n"
                                   + "# Allow changing the node_modules directory (eg: use nvm to manage node's version)\n"
                                   + "GLOBAL_NODE_MODULES=`npm root -g`"
                                   + "\n\n"
                                   + "node \"$GLOBAL_NODE_MODULES/cob-cli/node_modules/@commitlint/cli/lib/cli.js\" \\\n"
                                   + `\t-g ${path.resolve(process.cwd(), ".commitlintrc.js")} \\\n`
                                   + "\t-e \"$1\"\n");
