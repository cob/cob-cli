...
const COB_PRODUCTS = ["recordm", "integrationm", "recordm-importer", "logm", "userm", "devicem"];
...
/* ************************************ */
async function confirmExecutionOfChanges(server) {
    process.stdout.write("\nChecking changes... ")
    let changes = await _syncFiles(_syncFiles.DIFF, server,  "localMaster", "productionLive")
    if(changes.length) {
...
/* ************************************ */
async function _syncFiles(executionType, server, from, to) {
    let changes = []
...
function _formatRsyncOutput(product,rsyncOutput) {
    return rsyncOutput
        ...
        .filter(line => !line.startsWith(".")) // inalterados não interessam
        .map(line => {
