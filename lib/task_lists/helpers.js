...
async function registerRelease(server) {
    ...
    const date = new Date();
    var currentSha = await git().revparse(["--short", "HEAD"]).then(result => result);
    
    try {
        originalChangeLog = fs.readFileSync(changeLogFile).toString(); //TODO: consider getting file from server instead
    } catch (e) {
        // Assume non-existing file so procedd with default empty originalChangeLog
    }
    
    const newDeployText = `# Deploy ${currentSha} to ${server}\n` 
        + `\n Deployed by ${user}`
        + `\n ${date}\n`
    ...
    if (!matchResult) {
        ...
        if (lastSha == currentSha) {
            ...
            if (!diffs) {
                // No changes (excluding deploys) - do nothing
                return
            }
            diffs = "\n Previous deploy was __" + lastSha + "__. Changes:" + diffs;
        }
    }
    ...
    await git().add([changeLogFile.replace("#","chore:")])
