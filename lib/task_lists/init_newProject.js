...
function newProjectTasks(serverName, server, projectName, repo, args) {
    return new Listr([
            ...
            {   title: "Get initial defaults",                         enabled: () => !args.legacy, task: () => copyFiles(server,"productionDefault","localCopy")                             },
            ...
            {   title: "semantic-release -e deploy_semanticReleaseConfiguration --no-ci",           task: () => semanticRelease(server)                                                               }
        ],{
