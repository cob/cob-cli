...
function newProjectTasks(serverName, server, projectName, repo, args) {
    return new Listr([
        ...
        {   title: `[REMOTE] mkdir ${SERVER_COB_CLI_DIRECTORY}`.bold,                           task: () => _createRemoteCobCliDir(server)  },
...
/* ************************************ */
function _createRemoteCobCliDir(server){
    return new Observable(async (observer) => {
        ...
        try{
            console.log(`Creating the directory ${SERVER_COB_CLI_DIRECTORY}`);
            await execa('ssh', [server, `${cmds.join(" && ")}`]);
        } catch (e){
            if(e.stderr.includes("Permission denied") || e.stderr.includes("Operation not permitted")){
                console.warn(`Got an autorization error, going to retry with 'sudo'`);
                await sudo(server, cmds, observer).catch( err => observer.error(err));
            } else{
                console.error(`Error creating the directory ${SERVER_COB_CLI_DIRECTORY}`, e);
                observer.error(e);
            }
        }
