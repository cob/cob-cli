...
const { setupGitHooks, cobSemanticRelease, syncFilesTasks } = require("./helpers");


function newProjectTasks(serverName, server, projectName, repo, args) {
    return new Listr([
            {   title: "Get legacy repository",                        enabled: () => args.legacy,  task: () => _getLegacyFiles(args.legacy, projectName)                                       },
            ...
            {   title: "Get initial defaults",                         enabled: () => !args.legacy, task: () => syncFilesTasks(server,"productionDefault","localMaster", syncFilesTasks.COPY)   },
            ...
            {   title: "Get current config",                                                        task: () => syncFilesTasks(server,"productionLive","localMaster", syncFilesTasks.COPY)      },
            ...
            {   title: "[remote] Put current config in production.checkout",                        task: () => syncFilesTasks(server,"localMaster","productionMaster", syncFilesTasks.COPY)    },
        ],{
...
/* ************************************ */
function _getLegacyFiles(legayFolder, projectName) {
    let tasks = [
        ...
        {   title: "git rm --ignore-unmatch -r devicem/db",                                     task: () => execa('git', ["rm", "devicem/db","--ignore-unmatch", "-r"])            },
        ...
        {   title: 'git tag -d <existing tags> ',                                               task: () => _gitDeleteLegacyTags()                                                 }
    ]
    return new Listr(tasks);
}

/* ************************************ */
async function _gitDeleteLegacyTags() {
