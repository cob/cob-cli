...
async function customUIsContinuosReload(cmdEnv, dashboard) {
    ...
    try {
        const tailwindcssInputPath = path.resolve(".") + "/recordm/customUI/css/input.css";
        const tailwindcssOutputPath = path.resolve(".") + "/recordm/customUI/css/output.css";
        dashboardPathIsDirectory = dashboard && fs.lstatSync(dashboardPath).isDirectory()
        console.log("\nWatching tailwindcss input.css -> output.css \n")
        commands.push({
            // run tailwindcss from cob-cli directory 
            command: [
                path.resolve(__dirname, '../../node_modules/.bin/tailwindcss'),
                "-w",
                "-i",
                tailwindcssInputPath,
                "-o",
                tailwindcssOutputPath
            ].join(" "), 
            name: 'tailwindcss watcher',
            prefixColor: "magenta" 
        }) 
    } catch {
        console.log("\nNo tailwindcs inputfile\n")
    }
