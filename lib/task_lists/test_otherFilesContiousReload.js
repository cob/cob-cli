...
async function otherFilesContiousReload(server) {
    ...
    let watcher = fs.watch('.', { recursive: true }, async (eventType, changedFile) => {
        if (
            ...
            await addFileToCurrentTest(server, changedFile, changedFiles, "Syncing".bgRed.bold + " " );
...
async function removeFileFromCurrentTest(server, changedFile) {
    await syncFile(server, changedFile);
    await execa('ssh', [server, "sed -i '/" + changedFile.replace("/", "\\/") + "/d'  " + SERVER_IN_PROGRESS_TEST_FILE]).catch( () => {/* ignore files removed by user */} );
...
/* ************************************ */
function checkNoTestsRunningLocally(server) {
    try {
