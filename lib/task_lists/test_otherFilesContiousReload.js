...
/* ************************************ */
async function otherFilesContiousReload(cmdEnv) {
    let watcher = {}
    ...
    await cmdEnv.applySpecific()
    let changes = await confirmExecutionOfChanges(cmdEnv,excludeTestFilesList.concat("--exclude=/customUI/"))

    /* ************************************ */
    let restoreChanges = async () => {
        if(watcher.process) watcher.process.close();
        await cmdEnv.undoSpecific()
