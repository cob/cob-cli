...
async function otherFilesContiousReload(server) {
    ...
    watcher.process = fs.watch('.', { recursive: true }, async (eventType, changedFile) => {
        ...
        let productDir = resolveCobPath(server, "serverLive", product).split(":")[1];
        let productScopeFilePath = changedFile.substring(changedFile.indexOf("/") + 1); // remove o directorio inicial do producto

        let isRemoteFile = await execa('ssh', [server, "[[ -f " + productDir + productScopeFilePath + " ]] && echo 'file' || echo '' " ])
            .then( r => {
