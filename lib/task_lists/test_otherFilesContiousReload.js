...
/* ************************************ */
async function otherFilesContiousReload(env) {
    let watcher = {}
    let changedFiles = new Set()

    let currentTestsFilesList = []
    await execa('ssh', [env.server, "cat " + SERVER_IN_PROGRESS_TEST_FILE])
    ...
    let changes = await confirmExecutionOfChanges(env,excludeTestFilesList.concat("--exclude=/customUI/"))
    ...
    let restoreChanges = async () => {
        ...
        if(changedFiles.size > 0) {
            ...
            for(let changedFile of changedFiles) {
                await removeFileFromCurrentTest(env.server, changedFile);
            }
            await execa('ssh', [env.server,"find " + SERVER_IN_PROGRESS_TEST_FILE + " -size 0 -delete "]) //Remove file if empty
    ...
    for(let change of changes) {
        ...
        await addFileToCurrentTest(env.server, changedFile, changedFiles, changeType, restoreChanges);
    }
    ...
    watcher.process = fs.watch('.', { recursive: true }, async (eventType, changedFile) => {
        ...
        let productDir = resolveCobPath(env.server, "serverLive", product).split(":")[1];
        let productScopeFilePath = changedFile.substring(changedFile.indexOf("/") + 1); // remove o directorio inicial do producto

        let isRemoteFile = await execa('ssh', [env.server, "[[ -f " + productDir + productScopeFilePath + " ]] && echo 'file' || echo '' " ])
            .then( r => {
        ...
        if (
            ...
            try {
                await addFileToCurrentTest(env.server, changedFile, changedFiles, " Syncing ".bgRed.bold + " ", restoreChanges );
