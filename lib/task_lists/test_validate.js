...
async function validateTestingConditions(server, args) {
    ...
    let result = await new Listr([
            ...
            {   title: "find out SHA for last-deploy on specified server",                 task: (ctx)  => _getLastDeployedSha(server).then( lastSha => ctx.lastSha = lastSha).catch( err => ctx.err = err ) },
            ...
            {   title: "Check last-deploy == serverLive".bold,  skip: ctx => !ctx.lastSha, task: (ctx)  => testEquality(server, "serverLive", "localCopy").then( changes => _handleTestEquality(ctx, changes, server)  ) },
...
/* ************************************ */
function _getLastDeployedSha(server) {
...
async function _handleTestEquality(ctx, changes, server) {
    if (changes.length != 0) {
        let offendingFiles = changes.map(f => f.substring(20)) //20 é a posição onde começa o nome dos ficheiros em cada linha
