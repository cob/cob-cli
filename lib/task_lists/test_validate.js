...
async function validateTestingConditions(cmdEnv, args) {
    ...
    let result = await new Listr([
            ...
            {   title: "Apply enviroment specifics".bold,       skip: ctx => !ctx.lastSha, task: ()     => cmdEnv.applyLastEnvironmentDeployedToServerChanges() },
            ...
            {   title: "Undo enviroment specifics".bold,        skip: ctx => !ctx.lastSha, task: ()     => cmdEnv.unApplyLastEnvironmentDeployedToServerChanges() },
