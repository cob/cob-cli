...
async function validateTestingConditions(env, args) {
    console.log("Checking test conditions for", env.serverStr );
    let result = await new Listr([
            ...
            {   title: "find out SHA for last-deploy on specified server",                 task: (ctx)  => _getLastDeployedSha(env).then( lastSha => ctx.lastSha = lastSha).catch( err => ctx.err = err ) },
            ...
            {   title: "Apply enviroment specifics".bold,       skip: ctx => !ctx.lastSha, task: (ctx)  => env.applySpecific() },
            
            {   title: "Check last-deploy == serverLive".bold,  skip: ctx => !ctx.lastSha, task: (ctx)  => testEquality(env, "serverLive", "localCopy").then( changes => _handleTestEquality(ctx, changes, env.server).catch( err => ctx.err = err.message )  ) },
            
            {   title: "Undo enviroment specifics".bold,        skip: ctx => !ctx.lastSha, task: (ctx)  => env.undoSpecific() },
...
/* ************************************ */
function _getLastDeployedSha(env) {
    return new Promise(async (resolve, reject) => {
        getLastDeployedSha(env.server)
            .then(lastSha => {
                if (lastSha) {
                    resolve(lastSha)
                } else {
