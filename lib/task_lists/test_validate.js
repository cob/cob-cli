...
function validateTestingConditions(server, args) {
    ...
    return new Listr([
        ...
        {   title: "Check server copy == production files",                            task: (ctx)  => testEquality(server, "productionMaster", "productionLive").catch( err => ctx.err = err /* TODO check if change is registerd in test file or exlude from test*/) },
        {   title: "git checkout -",                                                   task: ()     => git().checkout("master") }, 
