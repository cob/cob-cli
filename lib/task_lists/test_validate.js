...
function validateTestingConditions(server, args) {
    ...
    return new Listr([
        ...
        {   title: "find out SHA for last-deploy on specified server",                 task: (ctx)  => getLastDeployedSha(server).then(lastSha => { if (lastSha) { ctx.lastSha = lastSha } else { ctx.err = "If you're deploying to a new server do " + "cob-cli deploy -s <newServer> --force".yellow + " first, to initialize the server\n " + "Aborted:".bgRed + " no previous deploy detected on this server." } }) }, 
