...
async function validateTestingConditions(server, args) {
    ...
    let result = await new Listr([
            ...
            {   title: "Check last-deploy == serverLive".bold,  skip: ctx => !ctx.lastSha, task: (ctx)  => testEquality(server, "serverLive", "localCopy").then( changes => _handleTestEquality(ctx, changes, server).catch( err => ctx.err = err.message )  ) },
