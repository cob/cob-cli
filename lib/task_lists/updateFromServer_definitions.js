...
async function getViewsForDef(servername, definition, dataPath, user) {
	...
	const response = await axios.get(uri);
	 ...
	 let userViews = response.data.filter(v => v.user == user)

	 if(userViews.length == 0)
		return

    userViews = userViews.map(v => {
...
async function getDefinitionsFromServer(servername, queries, definitionsPath, viewsPath, customization, user, args) {
