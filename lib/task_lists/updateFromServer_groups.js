...
async function groupsFromServer(servername, groupsQuery, exportPath, customization) {
	await new Listr(
	[
		{   title: `(${customization}) Permissions: `.bold + "searching for groups with query: " + groupsQuery,   task: async ctx => ctx.groupIds = await getGroups(groupsQuery, servername) },
