//----------------- Enable do $calc nas definições relevantes ------------------------
cob.custom.customize.push(function (core, utils, ui) {
    const definitions = [
        "Business Apps",
        "Project Data Points"
        // Insert new Definitions with $calc fields here (names should quoted and comma separated)
    ];

    definitions.forEach((defName) => {
        core.customizeInstances(defName, (instance, presenter) => calc_automation(instance, presenter))
    });
});
...
function calc_automation(instance, presenter) {
    ...
    function registerAndExecuteCalculation() {
        ...
        registerExecuteCalculationsOnChanges(calculations)
        executeCalculations(calculations)
    }
    ...
    function getAllCalculationsFields() {
        ...
        function getCalculationArguments(calculationFp) {
            ...
            function getAllAplicableFpsForVarName(calculationFp, varName) {
                let result
                if(varName === "parent") {
                    result = presenter.findFieldPs(fp => fp.field.id === calculationFp.field.parent)
    ...
    //=========================================================
    function registerExecuteCalculationsOnChanges(calculations) {
            ...
            calculation => calculation.args.forEach( arg => {
                if(isNaN(arg)) {
                    if(arg.field.fieldDefinition.duplicable) {
                        ...
                        // TODO: remover listeners actuais (não sei se é necessário)
                        // TODO: registar para eventos de remoção (fazer o mesmo, ie, registerAndExecuteCalculation)
                    } else {
                        //No caso de campos normais só é necessário calcular tudo quando uma dependência muda
                        presenter.onFieldChange(arg, () => executeCalculations(calculations) )
                    }
                }
            })
        )
    ...
    //=========================================================
    function executeCalculations(calculations) {
        ...
        function evaluateExpression(calculation) {
            // Obter valores actualizados para variaveis
